...
シート名・見出し・判定ラベルが翻訳されます。**投稿本文と数値は収集したまま**です。戦略シートの一部の生成文はまだ韓国語のままです — [既知の制限](#既知の制限)を参照してください。
...
- **成長インサイト** と **10万への戦略** の生成文は実行時に組み立てられるため、`--lang en|ja` でも韓国語のまま出力されます。シート構造・見出し・ラベルは翻訳されます。
- 列幅は翻訳前に計算されるため、一部の英語見出しが列幅より長くなります。
